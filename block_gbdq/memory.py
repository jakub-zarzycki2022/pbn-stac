...
class QueuePER(ExperienceReplay):
    def __init__(self, capacity: int, prob_alpha: float = 0.6):
        """A Prioritised Experience Replay buffer.
        ...
        self.recent = []
        self.buffer = PriorityQueue(maxsize=capacity)
    ...
    def store(self, transition: Transition):
        ...
        if self.buffer.qsize() < self.capacity:
            self.buffer.put(PrioritizedItem(transition.reward, transition))
        else:
            _ = self.buffer.get()
